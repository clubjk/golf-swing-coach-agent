...
# Search sends summaries in chunks so the prompt stays under API context limits (~1M tokens).
SEARCH_BATCH_JSON_CHARS = 280_000
...
def cmd_search(args):
    ...
    if args.search_media == "videos":
        indexed = {k: v for k, v in indexed.items() if v.get("media_type") == "video"}
    elif args.search_media == "images":
        indexed = {k: v for k, v in indexed.items() if v.get("media_type") == "image"}

    if not indexed:
        prog = Path(sys.argv[0]).name
        if args.search_media != "all" and n_indexed_any_type > 0:
            print(
                f"No indexed {args.search_media} in the index "
                f"({n_indexed_any_type} other item(s) indexed). "
                f"Index videos with: python {prog} index --type videos "
                f"or python {prog} index-folder \"/path\" --type videos"
            )
            sys.exit(1)
        if not items:
            print(f"No index entries yet. Run: python {prog} index")
        else:
            by_status = Counter(v.get("status") or "?" for v in items.values())
            summary = ", ".join(f"{s}={n}" for s, n in sorted(by_status.items()))
            print(
                f"Index has {len(items)} entr{'y' if len(items) == 1 else 'ies'}, "
                f"but search needs items with status 'indexed' (you have: {summary})."
            )
            print(f"Index file: {INDEX_FILE}")
            if by_status.get("unavailable"):
                print(
                    "Those are iCloud placeholders with no local file yet. "
                    "Photos → Settings → iCloud → Download Originals to this Mac; "
                    "when done, run:\n"
                    f"  python {prog} index\n"
                    "Entries are retried automatically (no --reindex needed).\n"
                    f"Check download progress anytime: python {prog} index --probe"
                )
            elif by_status.get("error"):
                print(f"Retry failed items with: python {prog} index --reindex")
        sys.exit(1)
    ...
    batch_model = (args.batch_model or SEARCH_BATCH_MODEL).strip()
    rerank_model = (args.rerank_model or SEARCH_RERANK_MODEL).strip()

    print(f"Searching {len(indexed)} item(s) for: '{query}'")
    print(
        f"  Batch model: {batch_model}  |  Rerank: "
        f"{'(skipped)' if args.no_rerank else rerank_model}  |  Max results: {top}\n"
    )
    ...
    if n_batches > 1:
        print(
            f"Splitting into {n_batches} search batch(es) (API context limit); "
    ...
    for bi, batch in enumerate(batches, 1):
        ...
        if n_batches > 1:
            print(f"Asking {batch_model} (batch {bi}/{n_batches}, {len(batch)} item(s))...")
        else:
            print(f"Asking {batch_model} to find matches...")

        result_text = _search_stream_text(client, batch_model, prompt)

        try:
            batch_matches = parse_search_matches_json(result_text)
        except json.JSONDecodeError:
            print(f"Could not parse response for batch {bi}/{n_batches}. Raw output:")
            print(result_text[:8000] + ("..." if len(result_text) > 8000 else ""))
            sys.exit(1)
    ...
    if not args.no_rerank and len(matches) > top:
        ...
        r_text = _search_stream_text(client, rerank_model, r_prompt)
        try:
            reranked = parse_search_matches_json(r_text)
        except json.JSONDecodeError:
            print("Rerank parse failed; using batch ordering. Raw (truncated):")
            print(r_text[:4000] + ("..." if len(r_text) > 4000 else ""))
            matches = matches[:top]
        else:
            seen_r: set[str] = set()
            cleaned: list[dict] = []
